...
async function logTweetTimestamp() {
  const ts = Date.now();
  await update(ref(db, `tweetLog/${ts}`), true);
}
...
const TEMPLATES = [
  ...
  // ... (your full template list unchanged)
];
...
async function postTweet() {
  try {
    ...
    await logTweetTimestamp();
  } catch (error) {
    console.error('Promo tweet failed:', error);
  }
}

// --- Post Username Invite Tweet ---
